    ...
    public class StudentDL : IStudentDL
    {
       ...
       public IEnumerable <SubjectResponseDto> GetSubjects(int Id)
        ...
        public decimal GetStudentSubjectMarks(int Id,int SubjectId)
        {
            ...
            var studentSubject = _stContext.studentSubjectDbDto.FirstOrDefault(x=>x.SubjectId == SubjectId);
            if (studentSubject == null)
            {
                throw new Exception("Subject Not Found");
            }
            var marksRecord = _stContext.studentSubjectMarksDbDto.FirstOrDefault(x => x.StudentId == Id && x.SubjectId == SubjectId);
            if (marksRecord == null)
            {
                throw new Exception("Marks not found");
            }
            return marksRecord.Marks;
        }
